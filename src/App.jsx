...
export default function App() {
  ...
  const runDoctorMode = async () => {
    ...
    try {
      ...
      const soapNote = await callGemini(SOAP_PROMPT, rawNotes, 0.2);
      setSoapOutput(soapNote);

      setStatusText('Running FlagAgent + SummaryAgent...');
      setStatusText('Running FlagAgent...');
      const flags = await callGemini(FLAG_PROMPT, soapNote, 0.1);
  ...
  const renderFlags = () => {
    ...
    const items = flagsOutput.split(/(?=\b\d+\.\s*[🔴🟡🔵])/).map(s => s.trim()).filter(Boolean);
